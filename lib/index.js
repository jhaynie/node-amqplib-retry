(function () {
  ...
  function Constructor(options) {
    function getConnection(options) {
      if (options.amqplibConnection) {
        return options.amqplibConnection;
      } else {
        return amqp.connect(options.amqpUrl || 'amqp://guest:guest@localhost:5672');
      }
    }
    ...
    this.FAILURE_QUEUE_NAME = options.failureQueueName;
    this.DELAY_FUNCTION = options.delayFunction || getDelay;
  ...
  Constructor.prototype.retry = function (msg, targetQueueName) {
    ...
    return Promise.resolve(this.connection)
      ...
      .then(function () {
        ...
        expiration = this.DELAY_FUNCTION(data.properties.headers._retryCount);
        properties = {
          persistent: true,
          headers: {
        ...
        if (expiration < 1) {
          return channel.sendToQueue(this.FAILURE_QUEUE_NAME, new Buffer(data), properties);
        }
    ...
    return Promise.resolve(this.connection)
      ...
      .then(function () {
        return Promise.all([
          ...
          channel.checkQueue(this.FAILURE_QUEUE_NAME),
